...
namespace AdvancedPeopleSystem
{
    ...
    public class CharacterCustomization : MonoBehaviour
    {
        ...
        Vector3 feetOffsetVector = Vector3.zero;
        private void LateUpdate()
        {
            if (applyFeetOffset)
            {
                feetOffsetVector.y = feetOffset;
                originHip.localPosition = feetOffsetVector;
            }
        }
        ...
        public void SwitchCharacterSettings(int settingsIndex)
        {
            if (Settings.settingsSelectors.Count - 1 >= settingsIndex)
            {
                ...
                ResetAll();
                InitializeMeshes(selector.settings, false);
            }
        }
        /// <summary>
        ...
        public void InitializeMeshes(CharacterSettings newSettings = null, bool resetAll = true)
        {
            ...
            foreach(var obj in childTransforms)
            {
                ...
                if (objType != "ACCESSORY" && objType != "HAT" && objType != "PANTS" && objType != "SHIRT" && objType != "SHOES" && objType != "ITEM1" && objType.ToLower() != "hips")
                {
                    if (skinnedMesh == null)
                        continue;
                ...
                }else if(objType.ToLower() == "hips")
                {
                    ...
                    headHip = allHips.First(f => f.name.ToLower() == "head");
                ...
                else if(objType == "HAT" || objType == "SHIRT" || objType == "PANTS" || objType == "SHOES" || objType == "ACCESSORY" || objType == "ITEM1")
                {
                    ...
                    skinnedMesh.gameObject.SetActive(false);
                    var clothesAnchor = clothesAnchors.Find(f => f.partType.ToString().ToLower() == objType.ToLower());
                    if(clothesAnchor == null)
                    {
                        var newClothesAnchor = new ClothesAnchor();
                        newClothesAnchor.partType = (CharacterElementType)Enum.Parse(typeof(CharacterElementType), objType.ToLower(), true);
            ...
            else
            {
                ...
                foreach (var combinedMesh in combinedMeshes)
                {
                    if (combinedMesh.sharedMesh != null)
                    {
                        ...
                        {
                            if (typeName == combinedMesh.sharedMesh.GetBlendShapeName(i))
                            {
        ...
        private void SetHairByIndex(int index)
        {
            ...
            if (index != -1)
            {
                if (_settings.hairPresets.ElementAtOrDefault(index) == null)
                {
        ...
        private void SetBeardByIndex(int index)
        {
            ...
            if (index != -1)
            {
                if (_settings.beardPresets.ElementAtOrDefault(index) == null)
                {
        ...
        public void HideParts(string[] parts)
        {
            foreach (string p in parts)
            {
                ...
                {
                    if (cp.name.ToLower() == p.ToLower() && cp.skinnedMesh[0].enabled)
                    {
